...
export default function CommentSection({ softwareId }) {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const q = query(collection(db, "comments"), where("softwareId", "==", softwareId), orderBy("createdAt", "desc"))
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {comment.createdAt ? new Date(comment.createdAt).toLocaleDateString() : "Ahora"}
                  </span>
                </div>
